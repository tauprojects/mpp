...
package psy.lob.saw.synchrobench.hashmaps.lockfree;
 ...
 * {@link NonBlockingCliffHashMap} scales substatially better than
...
public class NonBlockingCliffHashMap<TypeK, TypeV> extends
	...
	// static{try{_unsafe = Unsafe.getUnsafe();} catch(Exception e)
	...
	static { // <clinit>
		Field f = null;
		try {
			f = NonBlockingCliffHashMap.class.getDeclaredField("_kvs");
	 ...
	 */
	public NonBlockingCliffHashMap() {
		this(MIN_SIZE);
	}
	 ...
	 */
	public NonBlockingCliffHashMap(final int initial_sz) {
	...
	public void clear() { // Smack a new empty table down
		Object[] newkvs = new NonBlockingCliffHashMap(MIN_SIZE)._kvs;
	...
	public Object clone() {
		try {
			...
			NonBlockingCliffHashMap<TypeK, TypeV> t = (NonBlockingCliffHashMap<TypeK, TypeV>) super
					.clone();
	...
	private static final Object get_impl(final NonBlockingCliffHashMap topmap,
		...
		while (true) {
			...
			if (++reprobe_cnt >= reprobe_limit(len) || // too many probes
					key == TOMBSTONE) {// found a TOMBSTONE key, means no more
										// keys in this table
				if (TRAVERSAL_COUNT) {
					finishCount(nodesTraversed);
				}
	...
	static void finishCount(int nodesTraversed) {
		Vars vars = counts.get();
		vars.nodesTraversed += nodesTraversed + 1;
		vars.getCount++;
	}
	...
	private static final Object putIfMatch(
			final NonBlockingCliffHashMap topmap, final Object[] kvs,
			final Object key, final Object putval, final Object expVal) {
		assert putval != null;
	...
	private static final class CHM<TypeK, TypeV> {
		...
		private final AtomicReferenceFieldUpdater<CHM, Object[]> _newkvsUpdater = AtomicReferenceFieldUpdater
		...
		private final Object[] resize(NonBlockingCliffHashMap topmap,
				Object[] kvs) {
		...
		// out
		// of the top position.
		private final void help_copy_impl(NonBlockingCliffHashMap topmap,
		...
		// read it yet.
		private final Object[] copy_slot_and_check(
				NonBlockingCliffHashMap topmap, Object[] oldkvs, int idx,
				Object should_help) {
			assert chm(oldkvs) == this;
			Object[] newkvs = _newkvs; // VOLATILE READ
		...
		private final void copy_check_and_promote(
				NonBlockingCliffHashMap topmap, Object[] oldkvs, int workdone) {
			assert chm(oldkvs) == this;
		...
		private boolean copy_slot(NonBlockingCliffHashMap topmap, int idx,
	...
	private class SnapshotV implements Iterator<TypeV>, Enumeration<TypeV> {
		...
		public SnapshotV() {
			while (true) { // Verify no table-copy-in-progress
				...
				// iterating.
				topchm.help_copy_impl(NonBlockingCliffHashMap.this, topkvs,
						true);
			}
		...
		Object key(int idx) {
			return NonBlockingCliffHashMap.key(_sskvs, idx);
		}
		...
		public void remove() {
			...
			putIfMatch(NonBlockingCliffHashMap.this, _sskvs, _prevK, TOMBSTONE,
					_prevV);
			_prevV = null;
		}
	...
	public Collection<TypeV> values() {
		return new AbstractCollection<TypeV>() {
			@Override
			public void clear() {
				NonBlockingCliffHashMap.this.clear();
			}

			@Override
			public int size() {
				return NonBlockingCliffHashMap.this.size();
			}

			@Override
			public boolean contains(Object v) {
				return NonBlockingCliffHashMap.this.containsValue(v);
			}
	...
	public Set<TypeK> keySet() {
		return new AbstractSet<TypeK>() {
			...
			public boolean contains(Object k) {
				return NonBlockingCliffHashMap.this.containsKey(k);
			}

			@Override
			public boolean remove(Object k) {
				return NonBlockingCliffHashMap.this.remove(k) != null;
			}
	 ...
	 * The {@link NonBlockingCliffHashMap} does not normally create or using
	...
	public Set<Map.Entry<TypeK, TypeV>> entrySet() {
		return new AbstractSet<Map.Entry<TypeK, TypeV>>() {
			...
			public boolean remove(final Object o) {
				...
				return NonBlockingCliffHashMap.this.remove(e.getKey(),
						e.getValue());
			}
